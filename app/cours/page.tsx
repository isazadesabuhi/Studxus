...
function MyCoursesContent() {
  ...
  const [activeFilter, setActiveFilter] = useState<TabKey>("reserves");

  // Support hash-based filtering (/cours/#reserves or /cours/#enseignes)
  useEffect(() => {
    const applyHash = () => {
      const hash = window.location.hash?.replace("#", "");
      if (hash === "enseignes" || hash === "reserves") {
  ...
  const handleFilterChange = (tab: TabKey) => {
    setActiveFilter(tab);
    ...
    const fetchData = async () => {
      try {
        ...
        if (!coursesResponse.ok) {
          throw new Error("Echec du chargement des cours");
        }
        const coursesData = await coursesResponse.json();

        const { data: sessionData } = await supabase.auth.getSession();
        let bookingsData = { bookings: [] as Booking[] };
        if (sessionData.session) {
          ...
          if (!bookingsResponse.ok) {
            throw new Error("Echec du chargement des reservations");
          }
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Etes-vous sur de vouloir supprimer ce cours ?")) return;
    try {
      const { data: sessionData } = await supabase.auth.getSession();
      if (!sessionData.session) throw new Error("Session expiree");
  ...
  const buildFirstSessionInfo = (
    ...
    if (!Array.isArray(sessions) || sessions.length === 0) {
      return { label: "Horaire a definir", availablePlaces: undefined };
    }
    ...
    const normalizedSessions = sessions
      .map((session) => {
        ...
        const startDate = new Date(
          String(startStr).includes("T") ? startStr : `${dateStr}T${startStr}`
        );
  ...
  const transformToCourseCard = (apiCourse: APICourse | Booking): Course => {
    ...
    const ownerId =
      ...
      (apiCourse as any)?.userId ??
      courseData?.author?.id ??
      (apiCourse as any)?.author?.id ??
      null;
  ...
  const teachingCourses =
    currentUserId != null
      ? courses.filter((c) => c.userId === currentUserId)
      : courses;
      ...
      {activeFilter === "enseignes" && (
        <div>
          {teachingCourses.length === 0 ? (
            <div className="text-center py-6">
              <p className="text-gray-600 mb-4">
                Vous n'avez pas encore cree de cours
              </p>
              <button
                onClick={() => router.push("/cours/creer-cours")}
